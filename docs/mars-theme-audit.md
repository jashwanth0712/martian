...
- Updated: 2026-04-28 — Controls area 3D props (remote control, joystick) replaced with Mars Research Station in LandingArea.js. Geodesic dome habitat with wireframe overlay, interior warm glow, airlock door with emissive strip, 4 solar panels with sun-tracking tilt animation and glint shader, antenna mast with blinking red beacon, 4 cyan dome rim LEDs, ground glow ring, 20 dust mote particles. Old GLB objects hidden by proximity to controlsInteractivePoint. Controls popup/menu functionality preserved unchanged.
- Updated: 2026-04-28 — Reset/bonfire area Mars-themed: sword GLB object hidden by name match, bonfire re-themed as signal beacon (blue particles, sienna scorch marks), label changed from "Res(e)t" to "R(e)start", achievement renamed to "Mission Reset". Circuit banners (Three.js/WebGL branding) replaced with canvas-drawn Mars mission labels ("ARES-1", "SOL ZERO", "JEZERO BASE"). Skull/skeleton kept as-is.
